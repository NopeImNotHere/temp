...
class LessonPaginator {
	...
	getDisplayElements(objectIndex) {
		...
		turnus.onchange = () => {
			changeInfoOfLesson(turnus, "FK_Turnus");
		}
		...
		schoolyear.onchange = () => {
			changeInfoOfLesson(schoolyear, "FK_Schuljahr");
		}
	 ...
	 * @memberof LessonPaginator
	 */
	getFachByTeacher(LessonID, TeacherID) {
		let lessonObjects = this.getObjectsByLessonID(LessonID);
		let Fach;
		lessonObjects[0].teachers.forEach(value => {
			if (value.FK_Lehrer = TeacherID) {
...
async function classLoadingProcess(SelectionProcess) {
	...
	SelectionProcess = new Number(SelectionProcess);
	console.log(SelectionProcess);
	dynamicHTMLObject.innerHTML = "";
	let h2 = document.createElement("h2");
	h2.value = new Number(SelectionProcess);
...
function classLessonEditorUI() {
	...
	let changeTeacherSubject = document.createElement("button");
	changeTeacherSubject.innerText = "Change Subject of Teacher";
	changeTeacherSubject.onclick = () => { changeSubjectOfTeacher() };
	footer.appendChild(changeTeacherSubject);
...
async function changeSubjectOfTeacher() {
	await waitForButtonPress({ buttonText: "Change Subject of Teacher from this Unterricht" }).then(async value => {
		const OverlayButton = value;
		const OverlayDiv = OverlayButton.parentNode;
		const table = OverlayButton.parentNode.previousSibling;
		...
		let subjectSelect = createSelect({
			dataObject: Data.Faecher,
			optionStructure: { title: "Fachkuerzel", label: "Fachbezeichnung", value: "PK_Fach" },
			id: "",
			placeholder: "--Select Subject--"
		});
		let chosenFachOption = await waitForChoice(subjectSelect, OverlayDiv);

		let PK_Unterricht_Lehrer = lessons.getUnterrichtTeacherPK(OverlayButton.value, chosenTeacherOption.value);

		simpleCalls("set_Unterricht_Lehrer_Fach", { PK_Unterricht_Lehrer: PK_Unterricht_Lehrer, FK_Fach: chosenFachOption.value });
	});
...
function changeInfoOfLesson(element, type) {
	...
	simpleCalls("set_Unterricht_" + type.replace("FK_", ""), { PK_Unterricht: PK_Unterricht, [type]: element.value })
}
