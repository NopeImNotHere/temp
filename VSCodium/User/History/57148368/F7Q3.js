...
function classLessonEditorUI() {
  ...
  for (let index = 1; index < checkedCheckboxes.length; index++) {
    ...
    let tableHeadings = ["Unterricht", "Wochenstunden", "Turnus", "Schuljahr", "Hinweis", "Schülergruppe", "LetztesUpdate", "Klassen", "Fächer", "Lehrer", "Stammraum", "Fachraum"];
    let headingElements = [];
    ...
    for (let index = 0; index < tableHeadings.length - 2; index++) {
      let row = LessonTableBody.insertRow();
      row.appendChild(headingElements[index]);
      if (dataElements[index] instanceof Array) {
        let cell = row.insertCell();

        if (dataElements[index][0].id == "teacher") {

          let lehrer_Table = document.createElement("table");
          (dataElements[index]).forEach(element => {
            let lehrerRow = lehrer_Table.insertRow();
            lehrerRow.appendChild(element);

            let subjectRoomRow = lehrer_Table.insertRow();
            let roomRow = lehrer_Table.insertRow();

            for (let i = tableHeadings.length - 2; i < tableHeadings.length; i++) {
              const element = array[i];
              console.log("TableHeadings element -2", element);
        ...
        row.insertCell().appendChild(dataElements[index])
      }
    }
